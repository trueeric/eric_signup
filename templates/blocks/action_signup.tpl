...
<div class="text-center my-2">
    <a href="<{$xoops_url}>/modules/eric_signup/index.php?op=eric_signup_data_create&action_id=<{$block.id}>" class="btn btn-lg btn-info <{if !( $block.enable && $block.number > $block.signup|@count && $xoops_isuser && $block.end_date|strtotime >= $smarty.now)}> disable <{/if}>"><i class="fa fa-plus" aria-hidden="true"></i> 立即報名</a>
</div>
